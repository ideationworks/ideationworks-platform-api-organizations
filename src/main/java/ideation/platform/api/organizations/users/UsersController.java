...
import ideation.platform.api.common.RequestResult;
import ideation.platform.api.logging.LogsService;
import ideation.platform.api.organizations.Organization;
import ideation.platform.api.organizations.OrganizationsRepository;
import ideation.platform.api.organizations.links.OrganizationLinksService;
import ideation.platform.api.organizations.roles.RolesService;
...
//            }
    ...
    public ResponseEntity<User> getById(@PathVariable("uuid") UUID uuid, Principal principal) {

        Optional<User> user = usersService.getByPrincipalOrganizationAndId(principal, uuid);

        return user.map(value -> new ResponseEntity<>(value, HttpStatus.OK)).orElseGet(() -> new ResponseEntity<>(HttpStatus.FORBIDDEN));
...
//        if (optionalUser.isPresent() && optionalUser.get().getIsAdmin()) {
...
//            MandrillUtil.sendPasswordReset(userResetRequest.getEmail(), baseUrl + "/login/reset/" + uuid.toString());
    ...
    @PostMapping(Patterns.UUIDv4)
    public ResponseEntity<User> updateByUserId(@PathVariable("uuid") UUID uuid, @RequestBody User user, Principal principal) {

        return new ResponseEntity<>(usersService.updateUserByPrincipal(uuid, user, principal).get(), HttpStatus.OK);
